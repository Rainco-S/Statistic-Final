...
bp <- barplot(
  ...
  ylab = "PM2.5均值（μg/m³）",
  col = "lightcoral",
  ylim = c(0, max(wd_stats$pm25_mean) * 1.2)  # 主轴范围留余量
)

par(new = TRUE)
plot(
  x = bp,  # 用条形的x轴坐标，确保折线与条形对齐
  y = wd_stats$wd_count,
  type = "b",  # 加圆点的折线，更清晰
  col = "darkblue",
  lwd = 2,
  ...
  axes = FALSE,
  xlab = "", ylab = "",
  ylim = c(0, max(wd_stats$wd_count) * 1.2)  # 次轴范围匹配频次最大值
)

axis(
  side = 4,
  at = seq(0, max(wd_stats$wd_count), by = 200),  # 刻度间隔根据你的数据调整
  labels = seq(0, max(wd_stats$wd_count), by = 200),
  family = "PingFang"
)
mtext("风向频次（次）", side = 4, line = 4)  # 次轴标签右移，避免重叠

legend(
  "topright",
  legend = c("PM2.5均值（主轴）", "风向频次（次轴）"),
  col = c("lightcoral", "darkblue"),
  pch = c(15, 16),  # 条形对应方块，折线对应圆点
  lty = c(NA, 1),   # 条形无线条，折线有线条
  bty = "n"  # 去掉图例边框，更美观
)
